...
static void echoEffect()
{
  ...
  double newData[sizeDataChunk],tempData[sizeDataChunk];
  printf ("channels is: %d, nSamples: is %d, sizeData is: %d\n", channels,nSamples,sizeDataChunk);
  ...
  while(audio.more_data_available()) {
	...
	//outfile.write_current_output(tempData[i]);
	if ((i+delay)<sizeDataChunk){ 
	   //data = data * 0.1; <-- efecto bajar audio.. jejejej 
		data = data + tempData[i+delay];
		i++;
	}
