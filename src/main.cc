...
static void echoEffect()
{
  ...
  int sizeDataChunk = audio.get_data_size();

  double sampleRate = audio.get_sample_rate_hz();
  unsigned int bitsPerSample = audio.get_bits_per_sample();
  double newData[sizeDataChunk];
	
  printf ("channels is: %d, sizeData is: %d\n", channels,sizeDataChunk);

  while(audio.more_data_available()){ 
    newData[i]=audio.read_current_input();  ;
	i++;
  }
  audio.reset_g_num_isamp();
  ...
  while(audio.more_data_available()) {
	...
	if ((i+delay)<sizeDataChunk){ 
		data = data + decay*newData[i+delay];
		i++;
	}
    outfile.write_current_output(data);
  }
  outfile.save_wave_file ("/home/dcarrera/source/gtk3/audio-player/Debug/src/fileEcho.wav");
  audio.reset_g_num_isamp();
...
static void volumenEffect()
{
  std::cout << "Volumen effect activated" << std::endl;	
  gint i=0;
  ...
  outfile.save_wave_file ("/home/dcarrera/source/gtk3/audio-player/Debug/src/fileVolumen.wav");
  audio.reset_g_num_isamp();
  std::cout << "Volumen effect applied" << std::endl;
}
...
static Glib::RefPtr<Gtk::Builder> get_builder(gint ui)
{
...
int main(int argc, char *argv[])
{
  ...
  buttonEffect2->signal_clicked().connect(sigc::ptr_fun(&volumenEffect));
