...
WAV_IN::set_filename(char *file_name)
{
  ...
  //create a new wav headers
  if (wav == NULL)
    {
      printf("cant new headers\n");
      exit(-1);
    }
  //create a new chunk headers
  if (chk == NULL)
    {
      printf("cant new headers\n");
      exit(-1);
  ...
  // from wav file header, compare 4 bits with WAVE value, from wID header
  for (i = 0; i < 4; i++)
    obuff[i] = wav->wID[i];
  obuff[4] = 0;
  ...
  // from wav file header, compare 3 bits with WAVE value, from fID header o fmt
  for (i = 0; i < 3; i++)
    obuff[i] = wav->fId[i];
  obuff[3] = 0;
  if (strcmp(obuff, "fmt") != 0)
    {
      printf("bad fmt format\n");
      exit(-1);
    }
